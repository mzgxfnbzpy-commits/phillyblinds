...
| Form backend | API path: Resend, recipients in `api/quote.js` TEAM_EMAILS. mailto fallback: same list | ✅ Active |
...
### Quote form email
**Every quote form on BOTH sites goes to the whole team (Justin, 2026-09-10):**

  justin@phillyblinds.com, sarah@phillyblinds.com, mike@phillyblinds.com, tarin@phillyblinds.com

Blindznation adds one more: `justin@blindznation.com`. Nothing goes to
`blindznation@gmail.com` any more — that address is retired.

The canonical list is `TEAM_EMAILS` at the top of `api/quote.js` (the Resend path).
The `mailto:` fallback links carry the identical comma-separated list, and the two
MUST be changed together. Where an address is also visible link text, the page shows
one readable brand address (`justin@phillyblinds.com` here, `justin@blindznation.com`
on Blindznation) while the href still reaches everyone.
